# -*- coding: utf-8 -*-
"""Python seed script for Qianfan website - bypasses Prisma's blocked query engine."""
import sqlite3
import bcrypt
from datetime import datetime

DB_PATH = "scripts/qianfan-seed.db"
...
def main():
    ...
    # Clear existing data
    tables = ["Banner", "ChatMessage", "ChatSession", "ContentPage", "Customer",
              "FAQ", "FollowUp", "Inquiry", "Product", "ProductImage", "Project",
              "Region", "Series", "Testimonial", "TrackingEvent", "User"]
    for t in tables:
    ...
    for f in faqs:
        cur.execute(
            'INSERT INTO "FAQ" (category, question, answer, keywords, sortOrder) VALUES (?,?,?,?,?)',
            f
        )
    ...
    for t in testimonials:
        cur.execute(
            'INSERT INTO "Testimonial" (customerName, company, country, rating, content, isPublished, sortOrder, createdAt) VALUES (?,?,?,?,?,?,?,?)',
            (t[0], t[1], t[2], t[3], t[4], 1, t[5], now())
        )
    ...
    for b in banners:
        cur.execute(
            'INSERT INTO "Banner" (title, subtitle, image, link, sortOrder, isPublished, createdAt, updatedAt) VALUES (?,?,?,?,?,?,?,?)',
            (b[0], b[1], b[2], b[3], b[4], 1, now(), now())
        )
    ...
    for c in content_pages:
        cur.execute(
            'INSERT INTO "ContentPage" (slug, title, content, metaTitle, metaDescription, updatedAt) VALUES (?,?,?,?,?,?)',
    ...
    for p in projects:
        cur.execute(
            'INSERT INTO "Project" (title, slug, description, content, location, projectDate, images, isPublished, sortOrder, createdAt, updatedAt) VALUES (?,?,?,?,?,?,?,?,?,?,?)',
            (p[0], p[1], p[2], p[3], p[4], p[5], "[]", 1, p[6], now(), now())
        )
